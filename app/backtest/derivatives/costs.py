...
class BacktestCostSimulator:
    ...
    def simulate_funding(
        self, timestamp: float, symbol: str, position_qty: float, mark_price: float
    ):
        if position_qty == 0:
            return
        ...
        self.funding_events.append(
            FundingEvent(
                timestamp=timestamp, symbol=symbol, amount=-cost  # Negative means paid
            )
        )
    ...
    def simulate_borrow(self, timestamp: float, asset: str, borrowed_amount: float):
        ...
        self.borrow_events.append(
            BorrowCostEvent(timestamp=timestamp, asset=asset, amount=-cost)
        )
