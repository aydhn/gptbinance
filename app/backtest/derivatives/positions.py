...
class BacktestDerivativePositionState:
    ...
    def apply_fill(
        self, symbol: str, is_buy: bool, quantity: float, price: float
    ) -> float:
        """
        ...
        if (
            current_qty == 0
            or (current_qty > 0 and is_buy)
            or (current_qty < 0 and not is_buy)
        ):
            new_qty = current_qty + signed_qty
            # VWAP entry
            new_entry = ((abs(current_qty) * current_entry) + (quantity * price)) / abs(
                new_qty
            )
