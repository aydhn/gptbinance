...
class AuctionFailuresManager:
    ...
    def evaluate(self, record_id: str) -> Dict[str, Any]:
        if record_id not in self.records:
            raise ClearingPlaneError(f"Record {record_id} not found in auction_failures")
        return {"status": "evaluated", "record": self.records[record_id]}

    def get_proof_notes(self, record_id: str) -> str:
        return f"Proof notes for auction_failures {record_id}"
