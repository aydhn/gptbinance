...
class AuctionPackagesManager:
    ...
    def evaluate(self, record_id: str) -> Dict[str, Any]:
        if record_id not in self.records:
            raise ClearingPlaneError(f"Record {record_id} not found in auction_packages")
        return {"status": "evaluated", "record": self.records[record_id]}

    def get_proof_notes(self, record_id: str) -> str:
        return f"Proof notes for auction_packages {record_id}"
