...
class ComparisonsManager:
    """
    Manager for comparisons in the collateral plane.
    Ensures strict governance and visibility.
    """
    def __init__(self):
        self.records: Dict[str, Any] = {}

    def register_comparisons(self, entity_id: str, data: Dict[str, Any]) -> str:
        if not entity_id:
            raise CollateralPlaneError("Entity ID required for comparisons")

        # Enforce no theater: ensure lineage refs are present
        data["lineage_refs"] = data.get("lineage_refs", []) + [f"registered_comparisons_{datetime.utcnow().isoformat()}"]
        self.records[entity_id] = data
        return entity_id

    def get_comparisons(self, entity_id: str) -> Dict[str, Any]:
    ...
    def evaluate_integrity(self, entity_id: str) -> List[str]:
        ...
        if not record:
            cautions.append(f"Missing comparisons record for {entity_id}")
