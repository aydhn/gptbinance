...
class DebtManager:
    """
    Manager for debt in the collateral plane.
    Ensures strict governance and visibility.
    """
    def __init__(self):
        self.records: Dict[str, Any] = {}

    def register_debt(self, entity_id: str, data: Dict[str, Any]) -> str:
        if not entity_id:
            raise CollateralPlaneError("Entity ID required for debt")

        # Enforce no theater: ensure lineage refs are present
        data["lineage_refs"] = data.get("lineage_refs", []) + [f"registered_debt_{datetime.utcnow().isoformat()}"]
        self.records[entity_id] = data
        return entity_id

    def get_debt(self, entity_id: str) -> Dict[str, Any]:
    ...
    def evaluate_integrity(self, entity_id: str) -> List[str]:
        ...
        if not record:
            cautions.append(f"Missing debt record for {entity_id}")
