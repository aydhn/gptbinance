...
class LiquidationManager:
    """
    Manager for liquidation in the collateral plane.
    Ensures strict governance and visibility.
    """
    def __init__(self):
        self.records: Dict[str, Any] = {}

    def register_liquidation(self, entity_id: str, data: Dict[str, Any]) -> str:
        if not entity_id:
            raise CollateralPlaneError("Entity ID required for liquidation")

        # Enforce no theater: ensure lineage refs are present
        data["lineage_refs"] = data.get("lineage_refs", []) + [f"registered_liquidation_{datetime.utcnow().isoformat()}"]
        self.records[entity_id] = data
        return entity_id

    def get_liquidation(self, entity_id: str) -> Dict[str, Any]:
