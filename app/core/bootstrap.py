...
def parse_args():
    ...
    parser.add_argument("--profile", type=str, choices=[p.value for p in EnvironmentProfile], help="Override profile")
    parser.add_argument("--check-only", action="store_true", help="Validate config and exit")
    parser.add_argument("--print-effective-config", action="store_true", help="Print config and exit")
    parser.add_argument("--bootstrap-storage", action="store_true", help="Create storage dirs and exit")
...
def bootstrap():
    ...
    if args.check_only: run_mode = AppRunMode.CHECK_ONLY
    elif args.print_effective_config: run_mode = AppRunMode.PRINT_EFFECTIVE_CONFIG
    elif args.bootstrap_storage: run_mode = AppRunMode.BOOTSTRAP_STORAGE
    ...
    # 5. Bootstrap Summary
    logger.info("Application bootstrapped successfully.", extra={
