...
class CanonicalResolver:
    ...
    def register(self, canonical_id: CanonicalId) -> None:
        self._mappings[canonical_id.entity_type][canonical_id.canonical_id] = canonical_id
        for alias in canonical_id.aliases:
            if alias in self._alias_index[canonical_id.entity_type]:
                ...
                if existing_canonical != canonical_id.canonical_id:
                     # ambiguous alias
                     self._mappings[canonical_id.entity_type][existing_canonical].is_ambiguous = True
                     canonical_id.is_ambiguous = True
            self._alias_index[canonical_id.entity_type][alias] = canonical_id.canonical_id

    def resolve(self, alias: str, entity_type: CanonicalEntityType) -> Optional[CanonicalId]:
