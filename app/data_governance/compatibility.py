...
class SchemaCompatibilityChecker:
    ...
    def check(
        self, from_schema: DataSchema, to_schema: DataSchema
    ) -> CompatibilityReport:
        ...
        if not diff_report.is_breaking:
            ...
            compatibility = (
                SchemaCompatibility.BACKWARD_COMPATIBLE
                if has_changes
                else SchemaCompatibility.FULLY_COMPATIBLE
            )
        ...
        if diff_report.is_breaking:
            breaking_changes.extend(
                [
                    f"Removed: {f}"
                    for f in diff_report.changes.get(SchemaChangeType.FIELD_REMOVED, [])
                ]
            )
            breaking_changes.extend(
                [
                    f"Type changed: {f}"
                    for f in diff_report.changes.get(SchemaChangeType.TYPE_CHANGED, [])
                ]
            )
