...
class CarryCostAccounting:
    ...
    def get_carry_cost_snapshot(
        self, product_type: ProductType, symbol: str
    ) -> CarryCostSnapshot:
        base_asset = symbol.replace("USDT", "")  # Very naive, just for structure
        quote_asset = "USDT"

        funding_total = self._accrued_funding.get(symbol, 0.0)
        borrow_total = self._accrued_interest.get(
            base_asset, 0.0
        ) + self._accrued_interest.get(quote_asset, 0.0)
