...
class LeverageManager:
    def __init__(self, registry: ProductRegistry):
        ...
        self._state: Dict[str, Dict[str, int]] = {} # product_type -> symbol -> leverage
    ...
    def set_leverage(self, product_type: ProductType, symbol: str, requested_leverage: int) -> int:
        desc = self.registry.get_descriptor(product_type)
        if not desc.capabilities.supports_leverage:
            raise InvalidLeverageChange(f"Product {product_type} does not support leverage.")

        max_allowed = desc.capabilities.max_leverage
        if requested_leverage > max_allowed:
            raise InvalidLeverageChange(f"Requested leverage {requested_leverage}x exceeds max allowed {max_allowed}x for {product_type}")
