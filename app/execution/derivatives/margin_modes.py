...
class MarginModeManager:
    def __init__(self, registry: ProductRegistry):
        ...
        self._state: Dict[str, Dict[str, MarginMode]] = {} # product_type -> symbol -> margin_mode
    ...
    def set_mode(self, product_type: ProductType, symbol: str, requested_mode: MarginMode) -> MarginMode:
        desc = self.registry.get_descriptor(product_type)

        if requested_mode not in desc.capabilities.supported_margin_modes:
            raise InvalidMarginModeTransition(f"Margin mode {requested_mode} is not supported for {product_type}")
