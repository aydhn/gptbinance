...
from .exceptions import (
    InvalidMarginModeTransition,
)  # Reusing exception type conceptually or create a new one if preferred
...
class PositionModeManager:
    ...
    def set_mode(
        self, product_type: ProductType, requested_mode: PositionMode
    ) -> PositionMode:
        ...
        if requested_mode not in desc.capabilities.supported_position_modes:
            raise InvalidMarginModeTransition(
                f"Position mode {requested_mode} is not supported for {product_type}"
            )
