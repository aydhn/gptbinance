...
class ReduceOnlyValidator:
    @staticmethod
    def validate(
        request: ReduceOnlyExecutionRequest,
    ) -> Tuple[ReduceOnlyVerdict, float, str]:
        """
        ...
        if current_qty == 0:
            return (
                ReduceOnlyVerdict.REJECTED,
                0.0,
                "Cannot reduce position: current position is 0.",
            )
        ...
        if is_current_long and intent.side == DerivativeSide.LONG:
            return (
                ReduceOnlyVerdict.REJECTED,
                0.0,
                "Reduce-only long intent rejected: current position is long (would increase position).",
            )
        ...
        if not is_current_long and intent.side == DerivativeSide.SHORT:
            return (
                ReduceOnlyVerdict.REJECTED,
                0.0,
                "Reduce-only short intent rejected: current position is short (would increase position).",
            )

        abs_current_qty = abs(current_qty)
        if intent.quantity > abs_current_qty:
            return (
                ReduceOnlyVerdict.ADJUSTED,
                abs_current_qty,
                f"Reduce-only intent adjusted: {intent.quantity} -> {abs_current_qty} to prevent flipping position.",
            )
