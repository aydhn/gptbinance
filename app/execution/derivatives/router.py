...
class DerivativeRouter:
    def route(self, intent: DerivativeExecutionIntent, paper_mode: bool = False):
        ...
        if paper_mode:
            logger.info(f"Routing {intent.product_type} {intent.symbol} to PAPER execution.")
            # Dispatch to paper derivatives runtime
        else:
            logger.info(f"Routing {intent.product_type} {intent.symbol} to TESTNET/LIVE execution.")
