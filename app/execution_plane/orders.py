...
class OrderSpecValidator:
    ...
    def validate(spec: OrderSpec):
        ...
        if spec.order_type in [OrderType.LIMIT, OrderType.STOP_LOSS_LIMIT, OrderType.TAKE_PROFIT_LIMIT]:
            if not spec.price:
                 raise InvalidOrderSpecError(f"Price is required for limit order types.")

        if spec.order_type in [OrderType.STOP_LOSS_LIMIT, OrderType.TAKE_PROFIT_LIMIT, OrderType.STOP_MARKET, OrderType.TAKE_PROFIT_MARKET]:
            if not spec.stop_price:
                 raise InvalidOrderSpecError(f"Stop price is required for stop order types.")
