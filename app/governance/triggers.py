...
class TriggerEvaluator:
    def evaluate_time_based(self, last_run: Optional[datetime], interval_hours: int) -> bool:
        if last_run is None:
            return True
        return datetime.now(timezone.utc) - last_run > timedelta(hours=interval_hours)

    def create_trigger(self, trigger_type: RefreshTriggerType, source: str) -> RefreshTrigger:
