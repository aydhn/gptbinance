...
from app.incident_plane.enums import (
    IncidentStatus,
    VerificationVerdict,
    IncidentSeverity,
)
...
class ClosureReadinessEvaluator:
    ...
    def assert_ready_for_closure(manifest: IncidentManifest) -> bool:
        if manifest.current_status != IncidentStatus.RESOLVED:
            raise InvalidClosureState(
                f"Incident {manifest.incident_id} must be in RESOLVED state before CLOSURE."
            )

        if (
            not manifest.verification
            or manifest.verification.verdict != VerificationVerdict.VERIFIED
        ):
            raise InvalidClosureState(
                f"Incident {manifest.incident_id} lacks successful recovery verification. Closure blocked."
            )

        if manifest.severity in [
            IncidentSeverity.SEV0_EMERGENCY,
...
class IncidentClosureEvaluator:
    def can_close(
        self, incident_id: str, linked_postmortem: PostmortemDefinition = None
    ) -> bool:
        if linked_postmortem:
            readiness = PostmortemClosureGovernance.evaluate_readiness(
                linked_postmortem
            )
