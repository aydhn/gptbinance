...
class IncidentPolicyChecker:
    ...
    def block_recovery(incident: IncidentRecord) -> List[str]:
        ...
        blockers = []
        if (
            incident.severity == IncidentSeverity.CRITICAL_INCIDENT
            and incident.state != "CONTAINED"
        ):
            blockers.append(
                "Critical incident must be fully contained before recovery can be planned."
            )
        return blockers
