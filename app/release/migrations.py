...
class MigrationExecutor:
    ...
    def create_plan(self, source: str, target: str, direction: MigrationDirection) -> MigrationPlan:
        path = self.registry.get_path(source, target)
        mig_ids = [m.id for m in path]
        sev = MigrationSeverity.LOW
        for m in path:
            if m.severity == MigrationSeverity.HIGH or m.severity == MigrationSeverity.DESTRUCTIVE:
                sev = m.severity
