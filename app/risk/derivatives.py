...
class DerivativeRiskController:
    ...
    def check_leverage_cap(
        self, product_type: ProductType, current_leverage: int
    ) -> bool:
        desc = self.registry.get_descriptor(product_type)
        if current_leverage > desc.capabilities.max_leverage:
            logger.warning(
                f"Risk Breach: Leverage {current_leverage}x exceeds product cap {desc.capabilities.max_leverage}x"
            )
            return False
        return True

    def check_liquidation_buffer(self, snapshot: LiquidationSnapshot) -> bool:
        if snapshot.proximity in (
            LiquidationProximity.WARNING,
            LiquidationProximity.DANGER,
        ):
            logger.warning(
                f"Risk Breach: Liquidation proximity is {snapshot.proximity.value} for {snapshot.symbol}"
            )
            return False
        return True

    def requires_strict_reduce_only(
        self, margin_mode: MarginMode, snapshot: LiquidationSnapshot
    ) -> bool:
