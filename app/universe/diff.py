...
class DiffEngine:
    def compute_diff(
        self, old_snap: UniverseSnapshot, new_snap: UniverseSnapshot
    ) -> UniverseDiff:
        ...
        old_all = to_set(
            old_snap.eligible_instruments
            + old_snap.caution_instruments
            + old_snap.blocked_instruments
        )
        new_all = to_set(
            new_snap.eligible_instruments
            + new_snap.caution_instruments
            + new_snap.blocked_instruments
        )
        ...
        # Eligibility changes
        old_elig_map = {
            f"{r.symbol}_{r.product_type.value}": "eligible"
            for r in old_snap.eligible_instruments
        }
        old_elig_map.update(
            {
                f"{r.symbol}_{r.product_type.value}": "caution"
                for r in old_snap.caution_instruments
            }
        )
        old_elig_map.update(
            {
                f"{r.symbol}_{r.product_type.value}": "blocked"
                for r in old_snap.blocked_instruments
            }
        )

        new_elig_map = {
            f"{r.symbol}_{r.product_type.value}": "eligible"
            for r in new_snap.eligible_instruments
        }
        new_elig_map.update(
            {
                f"{r.symbol}_{r.product_type.value}": "caution"
                for r in new_snap.caution_instruments
            }
        )
        new_elig_map.update(
            {
                f"{r.symbol}_{r.product_type.value}": "blocked"
                for r in new_snap.blocked_instruments
            }
        )
