...
from app.universe.models import ProductInstrument, UniverseProfileConfig, UniverseEligibilityResult
...
class ProfileEligibilityEvaluator(EligibilityEvaluator):
    def evaluate(self, instrument: ProductInstrument, profile_config: UniverseProfileConfig, profile: ProfileType) -> UniverseEligibilityResult:
        ...
        if instrument.ref.product_type not in profile_config.allowed_products:
            ...
            reasons.append(f"Product type {instrument.ref.product_type} not allowed in profile")
        ...
        # Freshness check
        if profile_config.require_fresh_metadata and instrument.freshness == MetadataFreshness.STALE:
        ...
        # Allow/Deny lists
        if profile_config.allowlist and instrument.ref.symbol not in profile_config.allowlist:
             verdict = EligibilityVerdict.BLOCKED
