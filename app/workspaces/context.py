...
class ContextManager:
    ...
    def require_active_context(self) -> WorkspaceContext:
        if self._active_context.status != ContextStatus.ACTIVE or not self._active_context.active_profile:
            raise WrongWorkspaceContextError("No active workspace context.")
        return self._active_context

    def set_active_context(self, workspace: WorkspaceConfig, profile: WorkspaceProfile) -> None:
        if profile.workspace_id != workspace.workspace_id:
            raise WrongWorkspaceContextError(f"Profile {profile.profile_id} does not belong to workspace {workspace.workspace_id}.")
