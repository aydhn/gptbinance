...
for path, name in integrations:
    # First, read the file and remove the dummy comments if they exist
    if os.path.exists(path):
        with open(path, "r") as f:
            lines = f.readlines()
        with open(path, "w") as f:
            for line in lines:
                if not line.startswith("# Escrow-plane integration:") and not line.startswith("# WARNING: Treated"):
                    f.write(line)
            # Append real code
