...
for file in files:
    ...
    if "packs.py" in file:
        ...
        content = content.replace(
            "from app.evidence_graph.models import EvidencePack, ArtefactRecord, RelationEdge\n",
    ...
    if "queries.py" in file:
        content = content.replace("from typing import List\n", "")
        content = content.replace(
            "from app.evidence_graph.models import QueryRequest, QueryResult, ArtefactRecord, RelationEdge\n",
    ...
    if "reporting.py" in file:
        ...
        content = content.replace(
            "from app.evidence_graph.models import ArtefactRecord, RelationEdge, EvidenceCaseFile, EvidencePack, GraphGapFinding, RedactionRecord\n",
    ...
    if "relations.py" in file:
        ...
        content = content.replace(
            "from app.evidence_graph.exceptions import InvalidRelationEdgeError\n", ""
        )

    if "history.py" in file:
        content = content.replace(
            "from app.evidence_graph.models import ArtefactRecord, RelationEdge\n", ""
        )
    ...
    if "ingest.py" in file:
        ...
        content = content.replace(
            "from app.evidence_graph.models import ArtefactRecord, ArtefactScope, ArtefactDescriptor\n",
            "from app.evidence_graph.models import ArtefactRecord, ArtefactDescriptor\n",
        )

    if "repository.py" in file:
        content = content.replace(
            "from app.evidence_graph.scopes import ScopeEnforcer\n", ""
        )
    ...
    if "models.py" in file:
        content = content.replace(
            "from app.evidence_graph.enums import EvidenceGraphVerdict\n", ""
        )
