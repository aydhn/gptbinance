import pytest
from app.collateral_plane.segregation import SegregationManager

def test_segregation_registration():
    manager = SegregationManager()
    entity_id = manager.register_segregation("TEST_001", {"data": "valid"})
    assert entity_id == "TEST_001"

    record = manager.get_segregation(entity_id)
    assert record["data"] == "valid"
    assert "lineage_refs" in record
    assert any("registered" in ref for ref in record["lineage_refs"])

def test_segregation_integrity():
    manager = SegregationManager()
    # Missing entity
    cautions = manager.evaluate_integrity("MISSING_001")
    assert len(cautions) > 0
