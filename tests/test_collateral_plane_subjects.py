import pytest
from app.collateral_plane.subjects import SubjectsManager

def test_subjects_registration():
    manager = SubjectsManager()
    entity_id = manager.register_subjects("TEST_001", {"data": "valid"})
    assert entity_id == "TEST_001"

    record = manager.get_subjects(entity_id)
    assert record["data"] == "valid"
    assert "lineage_refs" in record
    assert any("registered" in ref for ref in record["lineage_refs"])

def test_subjects_integrity():
    manager = SubjectsManager()
    # Missing entity
    cautions = manager.evaluate_integrity("MISSING_001")
    assert len(cautions) > 0
