...
def test_equivalence_clean():
    baseline = resolution_engine.resolve("test", ConfigScope(scope_class=ScopeClass.GLOBAL), [])
    target = resolution_engine.resolve("test", ConfigScope(scope_class=ScopeClass.GLOBAL), [])
...
def test_equivalence_degraded():
    ...
    target = resolution_engine.resolve("test", ConfigScope(scope_class=ScopeClass.GLOBAL), [source])
    report = evaluate_equivalence(baseline, target)
