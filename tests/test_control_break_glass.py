...
from app.control.enums import SensitiveActionType, OperatorRole, BreakGlassSeverity, AuthorizationVerdict
...
def test_break_glass_success():
    ...
    req = req_manager.create_request(SensitiveActionType.FLATTEN_LIVE_SESSION, requester, "test")

    bg_req = bg_manager.request_break_glass(req.id, BreakGlassSeverity.CRITICAL_RISK, "System bleeding")
...
def test_break_glass_not_allowed():
    ...
    req = req_manager.create_request(SensitiveActionType.DESTRUCTIVE_RETENTION_CLEANUP, requester, "test")

    bg_req = bg_manager.request_break_glass(req.id, BreakGlassSeverity.SYSTEM_DOWN, "Need space")
