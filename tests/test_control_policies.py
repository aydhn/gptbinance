...
def test_four_eyes_policy():
    ...
    # Self-approval
    record.decisions.append(ApprovalDecision(request_id=req.id, approver=requester, approved=True, timestamp=req.created_at))
    ...
    record.decisions.append(ApprovalDecision(request_id=req.id, approver=approver, approved=True, timestamp=req.created_at))
