...
def test_impact_analyzer():
    ...
    ref2 = DatasetRef(
        dataset_id="d2", dataset_type=DatasetType.NORMALIZED_DATA, version="v1"
    )
    ...
    assert (
        report.severity == ImpactSeverity.LOW
    )  # No downstreams > 0 in mock analysis logic without properly setting up node references matching IDs
