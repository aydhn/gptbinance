...
def test_tif_policy():
    allowed_limit = TIFPolicyEngine.get_allowed_tifs(
        OrderType.LIMIT, is_post_only=False
    )
    assert TIFClass.GTC in allowed_limit

    allowed_market = TIFPolicyEngine.get_allowed_tifs(
        OrderType.MARKET, is_post_only=False
    )
