...
class TestExposures(unittest.TestCase):
    def test_calculate_exposures(self):
        state1 = PositionStateManager.create_empty_state(
            "BTCUSDT", ProductClass.SPOT_PAIR, "sleeve-1"
        )
        PositionStateManager.apply_fill(
            state1,
            {"symbol": "BTCUSDT", "side": "long", "quantity": "1.0", "price": "50000"},
        )

        state2 = PositionStateManager.create_empty_state(
            "BTCUSDT", ProductClass.PERPETUAL, "sleeve-2"
        )
        PositionStateManager.apply_fill(
            state2,
            {"symbol": "BTCUSDT", "side": "short", "quantity": "0.5", "price": "50000"},
        )
        ...
        self.assertEqual(
            exposure.net_directional_exposure, Decimal("25000")
        )  # 50k - 25k
